...
func parameterAddToHeaderOrQueryEx(headerOrQueryParams interface{}, keyPrefix string, obj interface{}, style string, collectionType string) {
		...
		switch v.Kind() {
			...
			case reflect.Struct:
				if t,ok := obj.(MappedNullable); ok {
					...
					parameterAddToHeaderOrQueryEx(headerOrQueryParams, keyPrefix, dataMap, style, collectionType)
					return
				}
				if t, ok := obj.(time.Time); ok {
					parameterAddToHeaderOrQueryEx(headerOrQueryParams, keyPrefix, t.Format(time.RFC3339Nano), style, collectionType)
					return
				}
			...
			case reflect.Slice:
				...
				for i:=0;i<lenIndValue;i++ {
					...
					parameterAddToHeaderOrQueryEx(headerOrQueryParams, keyPrefixForCollectionType, arrayValue.Interface(), style, collectionType)
				}
				return
			...
			case reflect.Map:
				...
				for iter.Next() {
					...
					parameterAddToHeaderOrQueryEx(headerOrQueryParams, fmt.Sprintf("%s[%s]", keyPrefix, k.String()), v.Interface(), style, collectionType)
				}
				return
			...
			case reflect.Interface:
				fallthrough
			case reflect.Ptr:
				parameterAddToHeaderOrQueryEx(headerOrQueryParams, keyPrefix, v.Elem().Interface(), style, collectionType)
				return
